...
public class Service {
    ...
    private static final String ID_COLUMN_NAME = "fid";	
    ...
    public List<Map<String, Object>> findZonesInRadius(double lon, double lat, double distance) {
        ...
        return db.getZones().parallelStream().filter(f -> isFeatureInRadius(f, center, distance))
                .map(f -> createZone(f, center)).collect(Collectors.toList());
    }

    private Map<String, Object> createZone(SimpleFeature feature, Point center) {
        return new HashMap<String, Object>() {
            {
                put("id", feature.getAttribute(ID_COLUMN_NAME));
    ...
    public Map<String, Object> noise(double lon, double lat, List<SimpleFeature> list) {
        ...
        final SimpleFeature noiseFeature = list.parallelStream().filter((f) -> center.intersects(geometry(f)))
        ...
        return new HashMap<String, Object>() {
            {
                ...
                put("id", noiseFeature != null ? noiseFeature.getAttribute(ID_COLUMN_NAME) : null);
            }
        };
    }
    ...
    public Map<String, Object> atmosphere(double lon, double lat) {
        ...
        final SimpleFeature noiseFeature = db.getAtmosphere().parallelStream()
                .filter((f) -> center.intersects(geometry(f))).findFirst().orElse(null);
    ...
    public Map<String, Object> prices(double lon, double lat) {
        ...
        final SimpleFeature priceFeature = db.getPrices().parallelStream().filter((f) -> center.intersects(geometry(f)))
                .findFirst().orElse(null);

        return new HashMap<String, Object>() {
            {
                put("value", priceFeature != null ? findPrice(priceFeature.getAttribute("CENA")) : null);
                put("id", priceFeature != null ? priceFeature.getAttribute(ID_COLUMN_NAME) : null);
            }
        };
    }
    ...
    public Map<String, Object> flood(double lon, double lat) {
        ...
        final List<Map.Entry<Integer, List<SimpleFeature>>> bigWater = items.entrySet().stream()
                .filter((e) -> e.getKey() != 0).collect(Collectors.toList());
        bigWater.forEach((e) -> {
            ...
            result.put("q" + e.getKey(), new HashMap<String, Object>() {
                {
                    ...
                    put("id", numInt(feature.getAttribute(ID_COLUMN_NAME)));
                }
            });
        });
        ...
        if (smallWaterFeatures != null) {
            final SimpleFeature smallWater = smallWaterFeatures.stream()
                    .sorted(Comparator.comparingDouble((f) -> findDistance(center, f.getDefaultGeometry()))).findFirst()
                    .orElse(null);
            if (smallWater != null) {
                result.put("small", new HashMap<String, Object>() {
                    {
                        ...
                        put("dist", 0.0);
                        put("id", numInt(smallWater.getAttribute(ID_COLUMN_NAME)));
                    }
                });
            }
        }
        ...
        if (bigWater.isEmpty() && smallWaterFeatures == null) {
            final List<SimpleFeature> bigWaterFeatures = db.getFlood().parallelStream()
                    .filter((f -> numInt(f.getAttribute("Q")) != 0)).collect(Collectors.toList());
                ...
                result.put("q" + qValue, new HashMap<String, Object>() {
                    {
                        ...
                        put("id", numInt(f.getAttribute(ID_COLUMN_NAME)));
                    }
                });
            });
            ...
            result.put("small", new HashMap<String, Object>() {
                {
                    ...
                    put("id", numInt(smallWaterFeature.getAttribute(ID_COLUMN_NAME)));
            }});
        }
    ...
    private Point createPoint(double lon, double lat) {        
        return Utils.GEOMETRY_FACTORY.createPoint(new Coordinate(lon, lat));
    }
}
